...
<?php
    ...
    $getServices = $con->query("select duration.id, start_date, end_date from duration, `Services` where duration.sid = `Services`.id AND `Services`.service = '{$_GET['service']}' AND duration.uid = {$_GET['uid']} order by duration.id desc;")->fetch_assoc();
    define('START_DATE', $getServices['start_date']);
    define('END_DATE', $getServices['end_date']);
?>
    <html>
